...
import { UNIT_LABELS, UNIT_ORDER } from '../../_lib/utils.js';
...
export async function onRequestGet({ request, env }) {
  ...
  for (const row of rows.results || []) {
    ...
    const ext = row.image_type === 'image/jpeg' ? 'jpg' : row.image_type === 'image/webp' ? 'webp' : 'png';
    const baseName = sanitizeZipSegment(String(row.image_name || 'minh-chung').replace(/\.[^.]+$/, ''));
    files.push({
