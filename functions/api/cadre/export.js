...
export async function onRequestGet({ request, env }) {
  ...
  const task = await env.DB.prepare(
    `SELECT id, title FROM tasks WHERE id = ? LIMIT 1`,
  )
    .bind(taskId)
    .first();
  if (!task) {
  ...
  if (
    (rows.results || []).reduce(
      (n, r) => n + Number(r.image_size || 0) + 1024,
      0,
    ) >
    3.8 * 1024 ** 3
  )
    return Response.json(
      { error: "Task vượt 3,8 GB; hãy tải theo từng hồ sơ." },
      { status: 413 },
    );
  ...
  for (const row of rows.results || []) {
    const unitDir = sanitizeZipSegment(
      UNIT_LABELS[row.unit_code] || row.unit_code,
    );
